...
/* kqueue 后端 dpasc：LT + kevent.data(able_bytes) 配额驱动 post 循环。
 * - DPASC_GIO_FUNCTION：read/write/splice/tee 等通用 fd I/O（prep+post 循环）
 ...
 * QIC/SSL dpasc 在 dpqic.c / dpssl.c，CTC submit/TMR 在 dpevp_kqueue.c。 */
...
static dpret_t _dpaio_buf_post(dpele_t* gio, dpasc_out_t* out, _dpaio_data_f iofun)
{
    ...
    int fd = dpefd_fd(gio);
    int total = 0;
    int saved_len = ioarg->len;
    uint32_t quota = out->able_bytes;
    ssize_t res = 0;

    for (;;) {
        int remain = saved_len - total;
        int try_len = remain;
        if (quota > 0 && (uint32_t)try_len > quota) {
            try_len = (int)quota;
        }
        ioarg->len = total + try_len;
...
static dpret_t _dpaio_msg_post(dpele_t* e, dpasc_out_t* out, _dpaio_msg_f iofun)
{
    ...
    dpret_t ret = DPE_OK;
    for (;;) {
        if (quota == 0 && total < (int)total_len) {
            ret = DPE_WAIT;
            break;
        }
...
static dpret_t _dpefd_fallocate_prep(dpele_t* efd, va_list arg, dpasc_out_t* out)
{
    ...
    return _dpsyc_ret_ok(posix_fallocate(dpefd_fd(efd), (off_t)offset, (off_t)len));
}
